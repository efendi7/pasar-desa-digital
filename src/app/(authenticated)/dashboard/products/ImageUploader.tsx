    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      {slots.map((file, i) => (
        <div key={i} className="relative">
          ...
          <label
            htmlFor={`image-${i}`}
            className={`block aspect-square rounded-2xl border-2 border-dashed cursor-pointer transition-colors duration-300
              ${
                previews[i]
                  ? 'border-green-300 bg-green-50 dark:border-green-700 dark:bg-green-950/30'
                  : 'border-gray-300 bg-gray-50 hover:border-green-400 dark:border-zinc-700 dark:bg-zinc-900 dark:hover:border-green-500'
              }`}
          >
            {previews[i] ? (
              <div className="relative w-full h-full">
                ...
                {i === 0 && (
                  <div className="absolute bottom-2 left-2 px-2 py-1 bg-green-600 text-white text-xs rounded-lg shadow">
                    Utama
                  </div>
                )}
              </div>
