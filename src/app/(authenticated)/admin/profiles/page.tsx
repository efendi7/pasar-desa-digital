...
export default function ProfilesApprovalPage() {
  ...
  const pendingUsers = profiles.filter((p) => !p.is_active && !p.is_rejected);
  const processedUsers = profiles.filter((p) => p.is_active || p.is_rejected);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10">
      ...
      {/* === BAGIAN 2: USER SUDAH DISETUJUI / DITOLAK === */}
      <div>
        <h2 className="text-xl font-semibold text-gray-800 mb-4">
          Pengguna Disetujui / Ditolak
        </h2>
        {processedUsers.length === 0 ? (
          <p className="text-gray-500 text-center py-6">
            Belum ada pengguna yang disetujui atau ditolak.
          </p>
        ) : (
          <div className="overflow-x-auto bg-white shadow-md rounded-2xl border border-gray-100">
            <table className="min-w-full divide-y divide-gray-200">
